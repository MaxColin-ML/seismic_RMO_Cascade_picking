...
# Reference: 
# [1] H. Wang et al., "DSU-Net: Dynamic Snake U-Net for 2-D Seismic First Break Picking," in IEEE Transactions on Geoscience and Remote Sensing, vol. 62, pp. 1-13, 2024, Art no. 5926613, doi: 10.1109/TGRS.2024.3457521.
# [2] Y. Qi, Y. He, X. Qi, Y. Zhang and G. Yang, "Dynamic Snake Convolution based on Topological Geometric Constraints for Tubular Structure Segmentation," 2023 IEEE/CVF International Conference on Computer Vision (ICCV), Paris, France, 2023, pp. 6047-6056, doi: 10.1109/ICCV51070.2023.00558.
##########################################################
...
def get_coordinate_map_2D(
    ...
    y_offset_, x_offset_ = torch.split(offset, kernel_size, dim=1)   #the size of y_offset_ and x_offset_ are [B,K,W,H]

    y_center_ = torch.arange(0, width, dtype=torch.float32, device=device)#卷积核中心所在的位置的y坐标
    y_center_ = einops.repeat(y_center_, "w -> k w h", k=kernel_size, h=height)

    x_center_ = torch.arange(0, height, dtype=torch.float32, device=device)#卷积核中心所在位置的x坐标
    ...
    if morph == 0:
        ...
        y_offset_new_ = einops.rearrange(y_offset_new_, "k b w h -> b k w h")  #或许是卷积公式里的Δp，用来计算偏移量
