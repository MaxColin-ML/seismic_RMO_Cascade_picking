...
def test_main():
    ...
    parser.add_argument('--samp_list_npy', type=str, default=None)
    # feature setting
    ...
    parser.add_argument('--pred_num', type=int, default=-1)
    parser.add_argument('--mark_name', type=str, default='predict')
    args = parser.parse_args() 
    
    # =========== preparing test ===============
    # base path & name
    test_set_path = data_dict[args.test_set]
    save_name = '%s-%s-AGC=%s-rg=%d+%.1f+%.1f+%d-cluK=%.1f-%d' % (args.mark_name, args.test_set, args.new_agc_list, args.win_k, args.bw_data, args.bw_para, args.valid_range, args.clu_eps, args.min_len)
    config = args
    ...
    if config.samp_list_npy is None:
        sample_list = None
    else:
        sample_list = list(np.load(config.samp_list_npy))
    
    dl_lab = dataloader_lab(test_set_path, sample_list=sample_list)
    if config.pred_num > 0:
        np.random.seed(1)  # for reproducibility
        dl_lab.index_list = np.random.choice(
            dl_lab.index_list, 
            size=min(config.pred_num, len(dl_lab.index_list)), 
            replace=False
        )
    test_dl = DataLoader(
        dl_lab,
    ...
    # start test
    print('#', save_name)
    infer_class.test_step(test_dl, new_agc_list, save_name, config.pred_num, test_hyper_dict)
